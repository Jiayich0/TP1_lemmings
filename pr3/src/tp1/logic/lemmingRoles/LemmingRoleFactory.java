...
public class LemmingRoleFactory {
	...
	public static LemmingRole parse(String roleWords) throws RoleParseException {	// Parse(): Recorre la Lista de Roles
		for (LemmingRole role: availableRoles) {									// Si coincide nombre: devuelve Clase
			LemmingRole parsedRole = role.parse(roleWords);							// Si no: lanza Exception
			if (parsedRole != null) {
				return parsedRole;
            }
		}
		throw new RoleParseException(Messages.UNKNOWN_ROLE.formatted(roleWords));
	}
